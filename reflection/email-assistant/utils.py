...
print("Using EMAIL_SERVER_API_URL:", BASE_URL)
session = requests.Session()
session = requests.Session()
session.headers.update({
    "User-Agent": "LF-ADP-EmailClient/1.0",
...
def reset_database() -> dict:
    ...
    r = session.get(f"{BASE_URL}/reset/")
    r.raise_for_status()
    return r.json()
...
def test_send_email():
    ...
    print("------------------------Preloading sample emails into the database…")
    print(BASE_URL)
    
    print("Preloading sample emails into the database…")
    print(f"{BASE_URL}/send/")
...
def test_search_emails(q: str = "report"):
    r = session.get(f"{BASE_URL}/emails/search/", params={"query": q})
    return pretty_display(f"GET /emails/search?query={q}", r)
...
def test_filter_emails(recipient: str | None = None, date_from: str | None = None, date_to: str | None = None):
    ...
    if date_from:
        params["date_from"] = datetime.strptime(date_from, "%Y-%m-%d").isoformat()
    if date_to:
        params["date_to"] = datetime.strptime(date_to, "%Y-%m-%d").isoformat()
...
def call_llm_email_agent(prompt: str,
    ...
    Args:
        ...
        api_url: Base URL of the LLM server. If None, uses env var LLM_SERVER_URL.
    ...
    base = api_url or os.getenv("LLM_SERVER_URL")
    if not base:
        raise RuntimeError("LLM_SERVER_URL is not set. Put it in your .env (e.g., http://127.0.0.1:5001).")
